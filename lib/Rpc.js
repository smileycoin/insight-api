...
Rpc.getServiceAddresses = function(cb) {
    bitcoreRpc.getServiceAddresses(function(err, sa) {
        if (err) return cb(err);
        return cb(err, sa);
    });
};

Rpc.getOrgList = function(orglistId, cb){
    bitcoreRpc.getOrgList(orglistId, 
    function(err, orglist) {
	if(err) return cb(err);
	return cb(err, orglist);
    });
}

Rpc.getAllOrgLists = function(cb){
    bitcoreRpc.getAllOrgLists(
    	function(err, result) {
		if(err) return cb(err);
...
Rpc.getCouponList = function(couponlistid, cb){
    bitcoreRpc.getCouponList(couponlistid,
    	function(err, result) {
		if(err) return cb(err);
		return cb(err, result);
	});
...
Rpc.getAllCouponLists = function(cb){
    bitcoreRpc.getAllCouponLists(
	    function(err, result){
	    	if(err) return cb(err);
		return cb(err, result);
	    });
